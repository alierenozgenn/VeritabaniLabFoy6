# Redis Performance Test - Windows 10 Compatible
Write-Host "=== REDIS SIEGE TEST ===" -ForegroundColor Green
...
for ($i = 1; $i -le 10; $i++) {
    $job = Start-Job -ScriptBlock {
        ...
        # Her client 100 istek atar
        for ($j = 1; $j -le 100; $j++) {
            try {
                $response = Invoke-RestMethod -Uri "http://localhost:8080/nosql-lab-rd?student_no=2025000001" -Headers @{"Accept"="application/json"} -TimeoutSec 10
                if ($response) {
                    $clientSuccess++
                }
            } catch {
...
$result | Out-File -FilePath "redis-siege.results" -Encoding UTF8
Write-Host $result
Write-Host "Results saved to redis-siege.results" -ForegroundColor Yellow
